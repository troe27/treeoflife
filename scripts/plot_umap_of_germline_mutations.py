...
def load_germline_mutations(
    ...
    for line in open(fofn_path).readlines():
        ...
        sbs52_counts = df["COUNT"]
        sbs52_sum = sbs52_counts.sum()
        sbs52_frequencies = sbs52_counts / sbs52_sum
        ...
        sample_mutations[sample_name] = sbs52_frequencies

    # Convert to DataFrame
    sbs52_df = pd.DataFrame.from_dict(sample_mutations, orient="index")

    # # Fill any missing trinucleotides with 0 (shouldn't happen with complete data)
    sbs52_df = sbs52_df.fillna(0)
    return sbs52_df, sample_names, ref_sample_lookup
...
def plot_umap_of_somatic_mutations(
    ...
    sbs52_df, sample_names, ref_sample_lookup = load_germline_mutations(
        fofn_path,
        hdp_samples,
    ...
    for i in n_neighbors:
        for j in min_dists:
            embedding, _umap_model = perform_umap_analysis(
                sbs52_df, n_neighbors=i, min_dist=j, standardize=True
            )
    ...
    # Perform dimensionality reduction
    # embedding, _umap_model = perform_umap_analysis(
    #     sbs52_df, n_neighbors=13, min_dist=0.2, standardize=True
    # )
